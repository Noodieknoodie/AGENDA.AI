// frontend/src/config/topics.ts
export const clientMeetingTopics = [
  { id: "market-update", label: "Market Update" },
  { id: "estate-plan", label: "Estate Plan Review" },
  { id: "insurance-review", label: "Insurance Review" },
  { id: "retirement-planning", label: "Retirement Planning" },
  { id: "tax-planning", label: "Tax Planning" },
  { id: "cash-flow", label: "Cash Flow Analysis" },
  { id: "college-planning", label: "College Planning" },
  { id: "investment-strategy", label: "Investment Strategy" },
  { id: "asset-allocation", label: "Asset Allocation" },
  { id: "philanthropy", label: "Charitable Giving" },
  { id: "risk-management", label: "Risk Management" },
  { id: "long-term-care", label: "Long-Term Care" },
  { id: "beneficiary-review", label: "Beneficiary Review" },
  { id: "next-steps", label: "Next Steps" }
];

export const generalMeetingTopics = [
  { id: "introduction", label: "Introduction" },
  { id: "objective", label: "Meeting Objective" },
  { id: "review", label: "Previous Items Review" },
  { id: "updates", label: "Project Updates" },
  { id: "discussion", label: "Discussion Points" },
  { id: "decisions", label: "Key Decisions" },
  { id: "action-items", label: "Action Items" },
  { id: "assignments", label: "Assignments" },
  { id: "timeline", label: "Timeline Review" },
  { id: "budget", label: "Budget Discussion" },
  { id: "challenges", label: "Challenges & Risks" },
  { id: "resources", label: "Resource Allocation" },
  { id: "next-meeting", label: "Next Meeting" },
  { id: "conclusion", label: "Conclusion" }
];